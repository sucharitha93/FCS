...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
  ...
  boolean similarTrees(Node<E> a, Node<E> b){
      /*1. both empty */
      ...
      if (a != null && b != null) 
          return (similarTrees(a.left, b.left) && similarTrees(a.right, b.right));  
  ...
  boolean isCompleteBTree(){
      ...
      for(Position<E> p:positions()){
          if(isInternal(p))
              cInternal++;
          if(isInternal(p)&&numChildren(p)==2)
              cNChild++;
      }
      if(cInternal.equals(cNChild))
          return true;      
      return false;
  }
  public Boolean isFullBTree(){
      ...
      if(count.equals((size()-1)/2))
          return true;
      return false;
                      
  }
  
  public LinkedQueue flatTree(){
      LinkedQueue<E> lq=new LinkedQueue<>();
      for(Position<E> p:positions()){
          lq.enqueue(p.getElement());
      }
      return lq;
  }
